...
impl Pipeline {
    ...
    fn create_layout(device: &Device) -> Result<vk::PipelineLayout, Box<dyn Error>> {
    ...
    fn create_pipeline(
        ...
        let vert_entry = load_shader(device, "shader/compiled/main_vert.spv")?;
        let frag_entry = load_shader(device, "shader/compiled/main_frag.spv")?;
        ...
        unsafe {
            device.device.destroy_shader_module(vert_entry, None);
            device.device.destroy_shader_module(frag_entry, None);
        }
