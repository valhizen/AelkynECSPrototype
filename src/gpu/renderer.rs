...
// D32_SFLOAT = 32-bit float depth, no stencil. Good precision for 3D scenes.
...
impl Renderer {
    pub fn new(window: &winit::window::Window) -> Result<Self, Box<dyn Error>> {
        ...
        // Create pipeline (push constants only, no descriptor sets).
        let pipeline = Pipeline::new(&device, &swapchain, DEPTH_FORMAT)?;

        // Create egui overlay renderer.
        let egui_renderer =
            EguiRenderer::new(&device, &allocator, swapchain.surface_format.format)?;

        tracing::info!("Renderer initialised successfully");
    ...
    /// Optionally renders an egui overlay on top.
    pub fn draw_frame<F>(
        &mut self,
        ...
        F: FnOnce(&ash::Device, vk::CommandBuffer, vk::PipelineLayout),
    {
        unsafe {
            ...
            // --- egui overlay pass (rendered without depth) ---
            if let Some((primitives, textures_delta, pixels_per_point)) = egui_output {
                ...
                let egui_rendering = vk::RenderingInfo::default()
                    .render_area(vk::Rect2D {
                        offset: vk::Offset2D { x: 0, y: 0 },
                        extent,
                    })
                    .layer_count(1)
                    .color_attachments(std::slice::from_ref(&egui_color_attach));

                self.device.device.cmd_begin_rendering(cmd, &egui_rendering);
            ...
            // Transition the swapchain image into present layout.
...
impl Drop for Renderer {
    fn drop(&mut self) {
        ...
        // Destroy command pool and swapchain.
        self.swapchain.destroy(&self.device);
        self.commands.destroy(&self.device);

        // Explictly drop the egui resources before the device/allocator drops.
